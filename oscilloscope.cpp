...
static void on_process(void *userdata) {
    ...
    if (buf->datas[0].data) {
        ...
        int16_t* p = static_cast<int16_t*>(buf->datas[0].data);

        // Append new samples to the temporary buffer
        data->tempBuffer.insert(data->tempBuffer.end(), p, p + n_samples);
        data->bufferCount += n_samples;

        // When we have enough samples, write them to the audioBuffer
        if (data->bufferCount >= TOTAL_SAMPLES) {
            audioBuffer.write(data->tempBuffer.data());
            data->tempBuffer.clear();
            data->bufferCount = 0;
        }
    }
...
int main() {
    ...
    // Check if the HOME environment variable was successfully retrieved
    ...
    // Define the rest of the path for the config file
    std::string config_path_suffix = "/.config/oscilloscope.conf";

    std::string full_path = home_dir + config_path_suffix;
    ...
    while (running) {
        ...
        switch (currentMode) {
            ...
            case VU_METER:
                ...
                break;
            case BAR_GRAPH:
                drawBarGraph(width, draw_height, leftAudio, rightAudio, colorPairIDs, audio_stream_active);
                break;
            default: break;
        }
        attron(A_BOLD);
    ...
    // --- Cleanup ---
    #ifdef USE_PIPEWIRE
    // The PipeWire backend can be shut down gracefully, so we wait for its thread to finish.
    if (audioThread.joinable()) {
        audioThread.join();
    }
    #else
    // The PulseAudio pa_simple_read() call can block indefinitely, causing a freeze on exit.
    // Detaching the thread is a pragmatic solution that allows the main UI to exit cleanly,
    // and the OS will handle the termination of the detached audio thread.
    if (audioThread.joinable()) {
        audioThread.detach();
    }
    #endif
