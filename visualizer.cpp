...
void resizeVisualizerState(std::vector<float>& state, size_t newSize) {
    if (state.size() != newSize) {
        state.assign(newSize, 0.0f);
    }
}

void computeMirrorSpectrum(const int16_t* leftData, const int16_t* rightData, std::vector<float>& magnitudes) {
    ...
    static std::vector<float> monoWindowed(static_cast<size_t>(sampleCount), 0.0f);
    ...
    for (int n = 0; n < sampleCount; ++n) {
        const float monoSample = (static_cast<float>(leftData[n]) + static_cast<float>(rightData[n])) / (2.0f * 32767.0f);
        const float window = 0.5f * (1.0f - std::cos(2.0f * PI * static_cast<float>(n) / static_cast<float>(sampleCount - 1)));
        monoWindowed[static_cast<size_t>(n)] = monoSample * window;
    }
    ...
    for (int bin = 0; bin < binCount; ++bin) {
        ...
        for (int n = 0; n < sampleCount; ++n) {
            const size_t idx = static_cast<size_t>(bin) * static_cast<size_t>(sampleCount) + static_cast<size_t>(n);
            const float sample = monoWindowed[static_cast<size_t>(n)];
...
void drawGalaxy(WINDOW *win, int width, int height, const int16_t* leftData, const int16_t* rightData, const std::vector<int>& colorPairIDs, bool audio_active) {
    ...
    static std::uniform_real_distribution<float> dis_spawn_jitter(-5.0f, 5.0f);
    ...
    if (audio_active && overall_amplitude > 0.05f) {
        ...
        for (int i = 0; i < particles_to_spawn && particles.size() < max_particles; ++i) {
            Particle p;
            p.x = base_spawn_x + dis_spawn_jitter(generator); // Spawn near center, symmetrically
            p.y = base_spawn_y;
 ...
 */
void drawOscilloscope(WINDOW *win, int width, int height, const int16_t* leftData, const int16_t* rightData, const std::vector<int>& colorPairIDs) {
...
/**
 * @brief Draws a mirrored dotted waveform centered on a horizontal axis.
 *
 * Left side of the graph shows lower frequencies, and right side shows higher
 * frequencies. Each column's mirrored height is driven by the corresponding
 * frequency-bin magnitude.
 */
void drawMirrorWave(WINDOW *win, int width, int height, const int16_t* leftData, const int16_t* rightData, const std::vector<int>& colorPairIDs, bool audio_active) {
    if (width <= 0 || height <= 0) {
        return;
    }

    constexpr int spectrumBins = BUFFER_FRAMES / 2;
    constexpr int firstUsableBin = 1; // Skip DC bin to avoid left-edge bias.
    constexpr int lastUsableBin = spectrumBins - 1;
    constexpr float frequencySpread = 1.85f;   // Spreads low frequencies across more columns.
    constexpr float amplitudeCompression = 0.45f; // Lifts quieter bins for a wider look.
    static std::vector<float> displayAmplitudes;
    static std::vector<float> smoothedAmplitudes;
    static std::vector<float> spectrumMagnitudes;
    static float spectrumPeak = 0.01f;
    resizeVisualizerState(displayAmplitudes, static_cast<size_t>(width));
    resizeVisualizerState(smoothedAmplitudes, static_cast<size_t>(width));

    if (audio_active) {
        computeMirrorSpectrum(leftData, rightData, spectrumMagnitudes);
        float framePeak = 0.0f;
        if (!spectrumMagnitudes.empty()) {
            auto peakBegin = spectrumMagnitudes.begin();
            if (spectrumMagnitudes.size() > 1) {
                peakBegin = spectrumMagnitudes.begin() + 1;
            }
            framePeak = *std::max_element(peakBegin, spectrumMagnitudes.end());
        }
    ...
    const float rise_factor = 0.55f;
    for (int x = 0; x < width; ++x) {
        float currentAmplitude = 0.0f;
        if (audio_active && !spectrumMagnitudes.empty()) {
            if (width == 1) {
                const size_t singleBin = std::min(static_cast<size_t>(firstUsableBin), spectrumMagnitudes.size() - 1);
                currentAmplitude = spectrumMagnitudes[singleBin] / spectrumPeak;
            } else {
                const float xNorm = static_cast<float>(x) / static_cast<float>(width - 1);
                const float curvedPosition = std::pow(xNorm, frequencySpread);
                const float binPosition = static_cast<float>(firstUsableBin) +
                    curvedPosition * static_cast<float>(lastUsableBin - firstUsableBin);
                const int leftBin = static_cast<int>(binPosition);
                const int rightBin = std::min(leftBin + 1, lastUsableBin);
                const float blend = binPosition - static_cast<float>(leftBin);
                const float leftMagnitude = spectrumMagnitudes[static_cast<size_t>(leftBin)];
                const float rightMagnitude = spectrumMagnitudes[static_cast<size_t>(rightBin)];
                const float interpolated = leftMagnitude * (1.0f - blend) + rightMagnitude * blend;
                const float normalized = std::max(0.0f, interpolated / spectrumPeak);
                const float compressed = std::pow(normalized, amplitudeCompression);
                const float highFrequencyTilt = 1.0f + (xNorm * 0.30f);
                currentAmplitude = compressed * highFrequencyTilt;
            }
            currentAmplitude = std::clamp(currentAmplitude, 0.0f, 1.0f);
        }

        if (currentAmplitude > displayAmplitudes[x]) {
            displayAmplitudes[x] += (currentAmplitude - displayAmplitudes[x]) * rise_factor;
        } else {
            displayAmplitudes[x] = std::max(0.0f, displayAmplitudes[x] - decay__factor);
        }
    }

    if (width == 1) {
        smoothedAmplitudes[0] = displayAmplitudes[0];
    } else {
        for (int x = 0; x < width; ++x) {
            const int leftIndex = (x == 0) ? x : x - 1;
            const int rightIndex = (x == width - 1) ? x : x + 1;
            smoothedAmplitudes[x] = (displayAmplitudes[leftIndex] * 0.25f) +
                                    (displayAmplitudes[x] * 0.5f) +
                                    (displayAmplitudes[rightIndex] * 0.25f);
        }
    }

    const int centerY = height / 2;
    const int maxMirrorRows = std::max(0, std::min(centerY, height - centerY - 1));
    const chtype waveformGlyph = ACS_BULLET;

    for (int x = 0; x < width; ++x) {
        const float amplitude = std::clamp(smoothedAmplitudes[x], 0.0f, 1.0f);
        const int centerPairID = selectColorByAmplitude(amplitude * 0.25f, colorPairIDs);

        wattron(win, COLOR_PAIR(centerPairID));
        mvwaddch(win, centerY, x, waveformGlyph);
        wattroff(win, COLOR_PAIR(centerPairID));

        if (maxMirrorRows == 0 || amplitude <= 0.0f) {
            continue;
        }

        const int visibleRows = std::clamp(
            static_cast<int>(std::lround(amplitude * maxMirrorRows)),
            1,
            maxMirrorRows);

        for (int step = 1; step <= visibleRows; ++step) {
            const float segmentAmplitude = amplitude * (static_cast<float>(step) / visibleRows);
            const int colorPairID = selectColorByAmplitude(segmentAmplitude, colorPairIDs);
            const int upperY = centerY - step;
            const int lowerY = centerY + step;

            wattron(win, COLOR_PAIR(colorPairID));
            if (upperY >= 0) {
                mvwaddch(win, upperY, x, waveformGlyph);
            }
            if (lowerY < height) {
                mvwaddch(win, lowerY, x, waveformGlyph);
            }
            wattroff(win, COLOR_PAIR(colorPairID));
        }
    }
